"""
train_classifier_v7.py — V7 LightGBM classifier with:
  - Repo-level holdout split (80/20 by file count, random_state=42) — same 220-repo holdout as V4/V5/V6
  - Stage 1: binary C vs HDL classifier (unchanged from V5/V6)
  ...
  - 6 new port_mismatch-specific features appended to extract_features (same as V6)
  - NEW in V7: real port_mismatch examples filtered to HDL-only patches at load time
  - OOS classification report saved to v7_oos_report.json
  - Model saved to polaris_classifier_v7.pkl
"""

import os, json, collections, random, re
import numpy as np
import pickle
...
def patch_touches_hdl(patch_str):
    """Return True if the patch diff header references a .v or .sv file."""
    import re
    # Look for diff header lines like: --- a/foo.v, +++ b/bar.sv, diff --git a/x.v b/x.v
    hdl_pattern = re.compile(r'\.(v|sv)\b', re.IGNORECASE)
    for line in patch_str.splitlines():
        if line.startswith('--- ') or line.startswith('+++ ') or line.startswith('diff '):
            if hdl_pattern.search(line):
                return True
    return False
...
# Filter real port_mismatch examples to HDL-only patches (V7 new step)
pm_kept = 0
pm_dropped = 0
filtered_raw = []
for e in raw_examples:
    if e.get('label') == 'port_mismatch' and not e.get('is_synthetic', False):
        patch = e.get('patch', '')
        if not patch_touches_hdl(patch):
            pm_dropped += 1
            continue
        pm_kept += 1
    filtered_raw.append(e)
raw_examples = filtered_raw
print(f'port_mismatch HDL filter: kept {pm_kept} real examples, dropped {pm_dropped}')
...
# ---------------------------------------------------------------------------
_re_named_port = re.compile(r'\.\w+\(')
_re_port_width = re.compile(r'\[\d+:\d+\]')
_re_instantiation = re.compile(r'\w+\s+\w+\s*\(')
...
def extract_features(ex):
    ...
    # Changed lines (+/-) for port-mismatch features
    changed_lines = [
    ...
    # --- 6 new port_mismatch-specific features ---
    pm_named_port_refs = len(_re_named_port.findall(patch))
    pm_port_direction_changes = sum(
        1 for line in changed_lines
        if 'input' in line.lower() or 'output' in line.lower() or 'inout' in line.lower()
    )
    pm_port_width_refs = sum(
        len(_re_port_width.findall(line)) for line in changed_lines
    )
    pm_module_keyword = int(any('module ' in line.lower() for line in changed_lines))
    pm_instantiation_lines = sum(
        1 for line in changed_lines if _re_instantiation.search(line)
    )
    pm_port_dot_ratio = pm_named_port_refs / max(1, total_changed_lines)
    ...
    return [
        ...
        # --- appended port_mismatch features ---
        pm_named_port_refs,
        pm_port_direction_changes,
        pm_port_width_refs,
        pm_module_keyword,
        pm_instantiation_lines,
        pm_port_dot_ratio,
    ]
...
HAND_FEATURE_NAMES = [
    ...
    # appended V6 port_mismatch features
    'pm_named_port_refs', 'pm_port_direction_changes', 'pm_port_width_refs',
    'pm_module_keyword', 'pm_instantiation_lines', 'pm_port_dot_ratio',
]
...
# 6. Save model
# ---------------------------------------------------------------------------
model_path = Path('/Users/severinspagnola/Desktop/GithubCrawler/polaris_classifier_v7.pkl')
...
oos_report = {
    ...
    'pm_filter_kept': pm_kept,
    'pm_filter_dropped': pm_dropped,
}

oos_path = Path('/Users/severinspagnola/Desktop/GithubCrawler/v7_oos_report.json')

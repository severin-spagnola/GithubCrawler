"""
train_classifier_v4.py — V4 LightGBM classifier with:
  - Repo-level holdout split (80/20 by file count, random_state=42)
  - Stage 2 is 3-class: synthesis_error, port_mismatch, sensitivity_list
    (api_breaking removed from Stage 2; C examples still route via Stage 1)
  - HDL examples labeled api_breaking are DROPPED from Stage 2 training
  - OOS classification report saved to v4_oos_report.json
"""
...
# ---------------------------------------------------------------------------
# 5. Stage 2: HDL-only 3-class classifier (no api_breaking)
#    Train on HDL train examples with label in STAGE2_CLASSES
#    Drop HDL examples labeled api_breaking entirely
# ---------------------------------------------------------------------------
print('\n' + '='*60)
...
# 6. Save model
# ---------------------------------------------------------------------------
model_path = Path('/Users/severinspagnola/Desktop/GithubCrawler/polaris_classifier_v4.pkl')
...
# Stage 1 predictions on test set (already computed above as y1_pred)
combined_true = []
combined_pred = []
...
# For HDL examples truly labeled api_breaking in the test set:
# Stage 2 can't predict api_breaking anymore, so they'll be misclassified.
# Count them separately for transparency.
...
oos_path = Path('/Users/severinspagnola/Desktop/GithubCrawler/v4_oos_report.json')
